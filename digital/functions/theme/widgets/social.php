...
$socialProfiles = array('Twitter','Facebook', 'Google Plus', 'Instagram', 'Flickr', 'Vimeo', 'Youtube', 'Dribbble', 'Pinterest', 'Linkedin', 'Behance');
...
function widget( $args, $instance ) {
		...
		<ul class="social">
			...
			foreach ($socialProfiles as $item)
			{
				...
				if ($instance[$id] && $instance[$id] != '') {
				    ...
				    echo '<li><a class="'.$id.'" href="'.$instance[$id].'" rel="external,nofollow" title="'.$instance[$iditem].'"><img src="'. get_bloginfo('template_directory') .'/images/icons/'.strtolower($item).'.png" alt="'.$instance[$iditem]. '" /></a></li>'; 
				}
			} 
			?>
  		</ul>
    ...
    ?>
		...
		foreach ($socialProfiles as $item)
		{
			...
			?>
 	 		<p>
				<img style="float: left; margin:0 8px 0 0;" src="<?php echo bloginfo('template_directory') ?>/images/icons/<?php echo $id; ?>.png" />
 				<label for="<?php echo $this->get_field_id( $id ); ?>"><strong><?php echo $item; ?> URL</strong></label> 
				<input type="text"  id="<?php echo $this->get_field_id( $id ); ?>" name="<?php echo $this->get_field_name( $id ); ?>" value="<?php echo $instance[$id]; ?>" size="25"/> 

			</p> 
			<p style="margin-left:31px;"> 

 				<label for="<?php echo $this->get_field_id( $idtitle ); ?>">Label</label>
				<input type="text" id="<?php echo $this->get_field_id( $idtitle ); ?>" name="<?php echo $this->get_field_name( $idtitle ); ?>" value="<?php echo $instance[$idtitle]; ?>" size="25" /><br/>
			</p> 
